...
SECRET_KEY = os.getenv("JWT_SECRET", "supergeheimesjwt")
ALGORITHM = "HS256"
...
def get_current_user(credentials: str):
    """Überprüft, ob ein Benutzer eingeloggt ist und gibt seine Daten aus dem Token zurück."""
    ...
    # Überprüfen, ob das Token auf der Blacklist steht
    invalid = conn.execute("SELECT id FROM invalid_tokens WHERE token = ?", (credentials,)).fetchone()
    conn.close()
...
def create_token(user_id: int, name: str, role: str):
    """Erstellt ein JWT-Access-Token & ein Refresh-Token"""
    
    now = datetime.utcnow()
    
    # Access-Token mit Benutzerinformationen
