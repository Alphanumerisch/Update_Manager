...
def update_customer(customer_id: int, name: str, email: str, credentials: HTTPAuthorizationCredentials = Depends(security)):
    """
    📝 Aktualisiert den Namen und die E-Mail-Adresse eines Kunden in der Datenbank.
    """
    try:
        conn = get_db_connection()  # Verbindung zur Datenbank
        cursor = conn.cursor()
        ...
        return {"success": False, "message": f"Fehler beim Aktualisieren des Kunden: {str(e)}"}
...
def update_update(
    ...
    add_customer_id: Union[int, List[int]] = None,
    del_customer_id: Union[int, List[int]] = None
):
    """
    Aktualisiert ein Update. Optional können der Titel und die Beschreibung geändert,
    ein Kunde hinzugefügt und/oder ein Kunde entfernt werden.
    """
    print(f"🔍 update_update: Eingangsdaten: update_id={update_id}, titel={titel}, description={description}, add_customer_id={add_customer_id}, del_customer_id={del_customer_id}", file=sys.stderr)
    
    if not update_id:
        raise HTTPException(status_code=400, detail="Update-ID ist erforderlich.")
    
    # Falls keine Änderungen übergeben wurden, kann die Funktion abbrechen
    if not (titel or description or add_customer_id or del_customer_id):
        print("🔍 update_update: Keine Änderungen übergeben.", file=sys.stderr)
        return {"message": "Keine Änderungen übergeben."}
    
    # Falls einzelne Werte übergeben wurden, in Listen umwandeln
    if add_customer_id is not None and isinstance(add_customer_id, int):
        add_customer_id = [add_customer_id]
    if del_customer_id is not None and isinstance(del_customer_id, int):
        del_customer_id = [del_customer_id]
    
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Prüfen, ob das Update existiert
    cursor.execute("SELECT id FROM updates WHERE id = ?", (update_id,))
    update = cursor.fetchone()
    if not update:
        conn.close()
        print(f"❌ update_update: Update mit ID {update_id} nicht gefunden.", file=sys.stderr)
        raise HTTPException(status_code=404, detail="Update nicht gefunden.")
    
    # Aktualisiere Titel und/oder Beschreibung, wenn übergeben
    if titel or description:
        fields = []
        params = []
        if titel:
            fields.append("title = ?")
        ...
        try:
            sql = f"UPDATE updates SET {', '.join(fields)} WHERE id = ?"
            cursor.execute(sql, tuple(params))
            conn.commit()
            print(f"🔍 update_update: Update {update_id} aktualisiert mit titel={titel} und description={description}.", file=sys.stderr)
        except sqlite3.Error as e:
            conn.close()
            print(f"❌ update_update: Fehler beim Aktualisieren des Updates: {str(e)}", file=sys.stderr)
            raise HTTPException(status_code=500, detail=f"Fehler beim Aktualisieren des Updates: {str(e)}")
    
    # Kunden hinzufügen
    if add_customer_id:
        for customer_id in add_customer_id:
            # Prüfen, ob der Kunde existiert
            cursor.execute("SELECT id FROM customers WHERE id = ?", (customer_id,))
            customer = cursor.fetchone()
            if not customer:
                conn.close()
                print(f"❌ update_update: Kunde {customer_id} nicht gefunden.", file=sys.stderr)
                raise HTTPException(status_code=404, detail=f"Kunde {customer_id} nicht gefunden.")
            # Prüfen, ob der Kunde bereits zugewiesen ist
            cursor.execute("SELECT 1 FROM customer_updates WHERE update_id = ? AND customer_id = ?", (update_id, customer_id))
            if cursor.fetchone():
                print(f"🔍 update_update: Kunde {customer_id} ist bereits zu Update {update_id} zugewiesen.", file=sys.stderr)
            else:
                try:
                    cursor.execute("INSERT INTO customer_updates (update_id, customer_id) VALUES (?, ?)", (update_id, customer_id))
                    conn.commit()
                    print(f"🔍 update_update: Kunde {customer_id} wurde zu Update {update_id} hinzugefügt.", file=sys.stderr)
                except sqlite3.Error as e:
                    conn.close()
                    print(f"❌ update_update: Fehler beim Hinzufügen des Kunden {customer_id}: {str(e)}", file=sys.stderr)
                    raise HTTPException(status_code=500, detail=f"Fehler beim Hinzufügen des Kunden {customer_id}: {str(e)}")
    
    # Kunden entfernen
    if del_customer_id:
        for customer_id in del_customer_id:
            cursor.execute("SELECT 1 FROM customer_updates WHERE update_id = ? AND customer_id = ?", (update_id, customer_id))
            if not cursor.fetchone():
                print(f"🔍 update_update: Kunde {customer_id} ist nicht zu Update {update_id} zugewiesen.", file=sys.stderr)
            else:
                try:
                    cursor.execute("DELETE FROM customer_updates WHERE update_id = ? AND customer_id = ?", (update_id, customer_id))
                    conn.commit()
                    print(f"🔍 update_update: Kunde {customer_id} wurde von Update {update_id} entfernt.", file=sys.stderr)
                except sqlite3.Error as e:
                    conn.close()
                    print(f"❌ update_update: Fehler beim Entfernen des Kunden {customer_id}: {str(e)}", file=sys.stderr)
                    raise HTTPException(status_code=500, detail=f"Fehler beim Entfernen des Kunden {customer_id}: {str(e)}")
    
    conn.close()
    return {"message": "Update erfolgreich aktualisiert."}
