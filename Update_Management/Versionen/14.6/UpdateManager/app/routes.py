...
from app.models import RequestModel, RegisterModel, LoginModel, UpdateModel, BusinessTimeRequest, UpdateCustomerRequest
...
from .services import (
    ...
    get_update_info, check_permission, update_customer, update_update
)
...
def api_handler(request: RequestModel, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    # **🆕 NEU: update_customer Funktion in den API-Handler aufnehmen**
    elif request.action == "update_customer":
        return update_customer(request.customer_id, request.name, request.email, credentials)
    elif request.action == "update_update":
        return update_update(
            update_id=request.update_id,
            titel=request.title,
            description=request.description,
            add_customer_id=request.add_customer_id,  
            del_customer_id=request.del_customer_id       
        )
...
def refresh_token(request: Request, response: Response):
    ...
    try:
        ...
        conn.close()
        return {"message": "/refresh_token : Token erfolgreich erneuert", "Access Token": access_token}
...
def serve_module(module_name: str, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    # **Falls Token abgelaufen: Versuche, es zu erneuern**
    if not token_data:
        ...
        if refresh_token:
            ...
            if new_access_token:
                ...
                token_data = decode_token(new_access_token)  # 🔄 Neues Token verwenden
            else:
                print("❌ /module Endpunkt Debug: Refresh-Token ungültig")
                raise HTTPException(status_code=401, detail="Token abgelaufen, bitte erneut einloggen")
