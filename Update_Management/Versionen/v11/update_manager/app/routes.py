...
def api_handler(request: RequestModel):
    ...
    if request.token and request.action in [
        ...
        # **Neue Action für Kunden mit Token (get_business_time)**
    if request.action == "get_business_time":
        ...
        if not request.date or not request.start_time:
            raise HTTPException(status_code=400, detail="Datum und Startzeit erforderlich")
        
        # ✅ Korrekte Übergabe der Parameter
        return get_business_time(request.token, request.date, request.start_time)
    
    # **Neue Action get_update_info für Update-Details mit Token**
    elif request.action == "get_update_info":
        if not request.token:
            raise HTTPException(status_code=400, detail="Token erforderlich")
        
        return get_update_info(request.token)
