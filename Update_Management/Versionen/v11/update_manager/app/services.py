    ...
    try:
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            ...
            for customer in customers:
                update_link = f"http://192.168.168.160/update?token={customer['token']}"
...
def get_update_info(token: str): 
    """Gibt die Update-Informationen basierend auf dem Token zurück und prüft, ob bereits ein Termin gesetzt wurde"""

    conn = get_db_connection()

    # Prüfen, ob das Token existiert und gültig ist
    result = conn.execute("""
        SELECT 
            u.title, 
            u.description, 
            c.name AS customer_name,
            cu.selected_date -- Prüfen, ob bereits ein Datum gesetzt wurde
        FROM customer_updates cu
        JOIN updates u ON cu.update_id = u.id
        JOIN customers c ON cu.customer_id = c.id
        WHERE cu.token = ?
    ...
    if not result:
        raise HTTPException(status_code=404, detail="Ungültiges oder nicht vorhandenes Token")

    # Prüfen, ob ein Datum gesetzt ist
